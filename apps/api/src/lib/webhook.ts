...
export function buildLabelImageUrl(barcodeId: string): string {
  const base = process.env.APP_PUBLIC_URL ?? "http://localhost:3000";
  return `${base.replace(/\/$/, "")}/api/barcodes/${barcodeId}/label.png`;
}

// 순수 함수로 분리 — 네트워크/DB 없이 페이로드 구성 로직만 테스트할 수 있게 한다.
export function buildWebhookPayload(event: InventoryWebhookEvent, item: WebhookItem): InventoryWebhookPayload {
  ...
  return {
    ...
    labelImageUrl: primary ? buildLabelImageUrl(primary.id) : null,
...
export async function fireInventoryWebhook(event: InventoryWebhookEvent, item: WebhookItem): Promise<void> {
  ...
  const payload = buildWebhookPayload(event, item);
