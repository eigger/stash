...
- 연속 카메라 스캔: **입고(+1)** / **소비(−1)** 모드, 화면 전환 없이 계속 스캔 — 등록된 바코드는 수량 조정, 미등록 바코드는 자동 등록(외부 조회로 이름·사진 자동 채움)
- 대시보드: 총 자산가치, 재고부족·유통기한 임박 항목을 전면에
- 재고부족 항목으로 만드는 장보기 리스트 — 사서 `+`를 누르면 목록에서 자동으로 빠짐
- 검색·위치/카테고리 필터·정렬·페이지네이션이 있는 아이템 목록
- 대량 입력·스프레드시트 이전을 위한 CSV 가져오기 / 내보내기
- 라벨 인쇄: 단일 PNG, 또는 A4 라벨 시트 PDF(한글 이름은 번들된 Noto Sans KR 서브셋으로 렌더링)
- 유통기한 / 보증 만료 푸시 알림(Web Push)
- 휴지통(소프트 삭제) — 복구 + 30일 자동 영구삭제
- 오프라인 지원 PWA: 앱 셸 캐시 + 오프라인 스캔 큐(온라인 복귀 시 자동 동기화)
