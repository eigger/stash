...
- 중첩 위치(방 / 선반 / 상자)와 카테고리
- 수량, 재고부족 기준 수량, 유통기한·보증 만료일, 가격, 사진, 메모
- 통합 `Item` + `Barcode` 모델 — 기존 UPC/EAN, 직접 발급 내부 QR(아이템 딥링크), Matter 페어링 코드를 하나의 바코드 타입으로
- 연속 카메라 스캔: **입고(+1)** / **소비(−1)** 모드, 화면 전환 없이 계속 스캔 — 직접 등록 폼이나 바코드/Matter 코드 추가에서도 카메라 스캔 지원. 실사용 인식률/속도를 위해 포맷 제한·고해상도·연속 오토포커스로 튜닝했고, 스캔 성공 시 삑 소리·진동, 저조도용 플래시 토글 지원
- 외부 제품 조회 연동을 다중 선택 가능(Open Food Facts, UPCItemDB, 네이버쇼핑) — 원하는 제공자만 켜거나 아예 꺼서 조회 자체를 생략
- 대시보드: 총 자산가치, 재고부족·유통기한 임박 항목을 전면에
- 재고부족 항목으로 만드는 장보기 리스트 — 사서 `+`를 누르면 목록에서 자동으로 빠짐
- 검색·위치/카테고리 필터·정렬·페이지네이션이 있는 아이템 목록, 마지막 필터/정렬 기억; 여러 아이템을 선택해 위치/카테고리 일괄 변경·일괄 삭제
- 삭제 시 실행취소 — 아이템을 지우면 바로 **실행취소** 토스트가 뜸
- 대량 입력·스프레드시트 이전을 위한 CSV 가져오기 / 내보내기(바코드 값 포함)
- 라벨 인쇄: 단일 PNG, 또는 A4 라벨 시트 PDF(한글 이름은 번들된 Noto Sans KR 서브셋으로 렌더링)
- 유통기한 / 보증 만료 푸시 알림 + 주간 재고부족 요약 알림(Web Push)
- 휴지통(소프트 삭제) — 복구 + 30일 자동 영구삭제
- 오프라인 지원 PWA: 앱 셸 캐시, 스캔/직접 등록 홈 화면 숏컷, 오프라인 스캔 큐(온라인 복귀 시 자동 동기화)
- 프린터·라벨 기기 자동화(예: Home Assistant)용 재고 이벤트 웹훅
- 관리자 / 일반 역할, 최초 관리자 부트스트랩, 백업/복원, 한/영 i18n, 라이트/다크 테마
